...
header { 
    ...
    z-index: 9000; 
    display: flex; 
    justify-content: space-between; /* Spreads button and logo */
    align-items: center; 
    padding: 15px 40px; 
...
.logo-text { 
    ...
    color: var(--gold);
    position: absolute; /* Centers logo regardless of side buttons */
    left: 50%;
...
.step { background: rgba(255,255,255,0.03); border: 1px solid var(--border); padding: 25px; border-radius: 12px; width: 260px; text-align: center; transition: 0.3s; }
.step:hover { border-color: var(--gold); background: rgba(200, 164, 60, 0.05); transform: translateY(-5px); }
...
form { background: rgba(255,255,255,0.02); border: 1px solid var(--border); border-radius: 20px; padding: 45px; backdrop-filter: blur(15px); position: relative; transition: 0.4s; }
form:hover { border-color: var(--gb); box-shadow: 0 20px 40px rgba(0,0,0,0.4); }
...
input, select { width: 100%; background: rgba(255,255,255,0.05); border: 1px solid var(--border); color: var(--white); padding: 14px; border-radius: 6px; outline: none; transition: 0.3s; }

.btn-primary { padding: 18px 38px; background: var(--gold); color: var(--ink); font-size: 13px; font-weight: 800; letter-spacing: 2px; text-transform: uppercase; border-radius: 4px; transition: 0.3s; display: inline-block; border: none; cursor: pointer; width: 100%; margin-top: 35px; }
.btn-primary:hover { background: var(--gold-l); transform: translateY(-3px); box-shadow: 0 10px 30px rgba(200, 164, 60, 0.4); }
...
.success { background: rgba(22, 163, 74, 0.15); border: 1px solid #16a34a; color: #86efac; }
...
@media(max-width: 768px) {
    ...
    header { padding: 15px 20px; flex-direction: row; }
...
<header>
    <a href="Broker.php" class="back-btn">
        <span>←</span> Back
    </a>
    <div class="logo-text">HOUSING HUB BROKERS</div>
    <div style="width: 80px;"></div> <!-- Spacer to balance the flexbox -->
</header>
...
<main class="z">
    ...
    <div class="roadmap reveal" >
        <div class="step">
            ...
            <h3>Verification</h3>
            <p>Fill out the brokerage form below with your valid credentials.</p>
        </div>
        <div class="step">
            <span class="step-num">02</span>
            <h3>Review</h3>
            <p>Our concierge team validates your documents within 24 hours.</p>
        </div>
        <div class="step">
            <span class="step-num">03</span>
            <h3>Approval</h3>
            <p>Receive a "Verified Partner" confirmation via email & SMS.</p>
        </div>
        <div class="step">
            <span class="step-num">04</span>
            <h3>Dashboard</h3>
            <p>Login to your portal and start closing million-dollar deals.</p>
        </div>
    </div>
    ...
    <section id="verification">
      <div class="forms-container">
        <form id="personal-form" enctype="multipart/form-data" method="post" action="submit_verification.php" class="reveal">
        ...
        <form id="business-form" enctype="multipart/form-data" method="post" action="submit_verification.php" class="reveal">
    ...
    <div class="get-started-box reveal">
        ...
        <p style="color:var(--muted); margin-bottom:30px;">Proceed to the portal to manage your empire.</p>
        <a href="index.php" class="btn-primary" style="width:auto; padding: 20px 60px;">Enter Dashboard</a>
    </div>
</main>

<footer style="text-align:center; padding:60px; color:rgba(255,255,255,0.2); font-size:11px; letter-spacing:2px; position:relative; z-index:10;">
    &copy; 2026 HOUSING HUB | THE STANDARD OF LUXURY REAL ESTATE
</footer>
...
document.querySelectorAll('a, button, input, select, form').forEach(function(el){
